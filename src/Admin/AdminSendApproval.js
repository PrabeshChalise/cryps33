...
const AdminSendApproval = () => {
  ...
  const fetchSendRequestsForAdmin = async (adminId) => {
    try {
      const sendResponse = await axios.get(
        `http://localhost:3001/api/send-requests-for-admin/${adminId}`
      );
      const userResponse = await axios.get("http://localhost:3001/api/clients");
  ...
  const handleUpdateStatus = async (id, status) => {
    try {
      await axios.post(`http://localhost:3001/api/send-requests/${id}/status`, {
        status,
      });
