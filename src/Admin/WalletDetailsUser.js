...
const WalletDetailsUser = ({ selectedUserId }) => {
    ...
    const fetchWallet = async () => {
      try {
        const response = await axios.get(
          `http://localhost:3001/api/wallet/${selectedUserId}`
        );
  ...
  useEffect(() => {
    const fetchProfitLoss = async () => {
      try {
        const response = await axios.get(
          `http://localhost:3001/api/profit-stats/${selectedUserId}?period=1M`
        );
  ...
  useEffect(() => {
    const fetchWithdrawals = async () => {
      try {
        const response = await axios.get(
          `http://localhost:3001/api/send1/${selectedUserId}`
        );
  ...
  const handleUpdateCoinValue = async (e) => {
    e.preventDefault();
    try {
      await axios.post("http://localhost:3001/api/wallet3", {
