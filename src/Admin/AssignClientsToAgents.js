...
const AssignClientsToAgents = () => {
    ...
    const fetchAgents = async () => {
      try {
        const response = await axios.get(
          `http://localhost:3001/api/agents-for-admin/${adminId}`
        );
  ...
  const handleAssignClient = async (clientId) => {
    try {
      await axios.post("http://localhost:3001/api/assign-client", {
  ...
  const handleViewAssignedClients = async (agent) => {
    try {
      const response = await axios.get(
        `http://localhost:3001/api/assigned-clients/${agent._id}`
      );
  ...
  const handleRemoveClient = async (clientId) => {
    if (window.confirm("Are you sure you want to remove this client?")) {
      try {
        await axios.post("http://localhost:3001/api/remove-client", {
          agentId: selectedAgent._id,
          clientId: clientId,
        });
