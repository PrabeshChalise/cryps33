...
const TradeControl = () => {
  ...
  const fetchPredictions = async (userId) => {
    try {
      ...
      const waitingResponse = await axios.get(
        `http://localhost:3001/api/predictions/waiting1?userId=${userId}`
      );
      ...
      const defaultResultResponse = await axios.get(
        `http://localhost:3001/api/users/${userId}/default-trade-result`
      );
      setDefaultTradeResult(defaultResultResponse.data.defaultTradeResult);

      // Fetch the agentUID
      const agentUIDResponse = await axios.get(
        `http://localhost:3001/api/agent-uid-by-user-id/${userId}`
      );
  ...
  const handleResult = async (id, result) => {
    try {
      await axios.post(
        `http://localhost:3001/api/prediction/${id}/result`,
        result
      );
  ...
  const handleDefaultControl = async (result) => {
    try {
      ...
      await axios.post(
        `http://localhost:3001/api/users/${selectedUserId}/default-trade-result`,
