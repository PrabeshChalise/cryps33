...
const AgentClientApproval = () => {
    ...
    const fetchRequests = async () => {
      try {
        ...
        const response = await axios.get(
          `http://localhost:3001/api/client-requests-for-admin/${adminId}`
        );
  ...
  const handleApprove = async (id, clientId) => {
    try {
      ...
      const clientResponse = await axios.get(
        `http://localhost:3001/api/user-by-short-id/${clientId}`
      );
      const fullClientId = clientResponse.data._id;

      // Proceed with the approval using the full client ID
      await axios.post(
        `http://localhost:3001/api/client-requests/${id}/approve`,
        {
  ...
  const handleDecline = async (id) => {
    try {
      await axios.post(
        `http://localhost:3001/api/client-requests/${id}/decline`
      );
      setRequests(requests.filter((request) => request._id !== id));
    } catch (error) {
