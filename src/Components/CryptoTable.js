          ...
          <tbody>
            {coins.map((coin) => (
              <tr key={coin.id}>
                ...
                <td>{coin.high_24h ? `$${coin.high_24h.toLocaleString()}` : "N/A"}</td>
                <td>{coin.low_24h ? `$${coin.low_24h.toLocaleString()}` : "N/A"}</td>
                <td>{coin.total_volume.toLocaleString()}</td>
                <td>{coin.market_cap.toLocaleString()}</td>
                <td>{coin.total_supply ? coin.total_supply.toLocaleString() : "N/A"}</td>
                <td>
                  <button className="trade-button">Login to Trade & Practice</button>
                </td>
              </tr>
            ))}
          </tbody>
