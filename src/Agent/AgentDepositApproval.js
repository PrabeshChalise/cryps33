...
const AgentDepositApproval = () => {
  ...
  const fetchPendingDeposits = async () => {
    ...
    try {
      const response = await axios.get(
        `http://localhost:3001/api/deposits-for-agent/${agentId}`
      );
  ...
  const fetchDepositsForUser = async (userId) => {
    try {
      const response = await axios.get(
        `http://localhost:3001/api/deposits/user/${userId}`
      );
      setPendingDeposits(response.data);
  ...
  const handleApprove = async (id, updatedAmount) => {
    ...
    try {
      await axios.post(`http://localhost:3001/api/deposits/${id}/approve`, {
  ...
  const handleDecline = async (id) => {
    try {
      await axios.delete(`http://localhost:3001/api/deposits/${id}`);
  ...
  const handleConfirmAction = async () => {
    ...
    try {
      await axios.post(
        `http://localhost:3001/api/deposits/${currentDeposit._id}/complete`
      );
