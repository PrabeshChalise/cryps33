...
const AdminInfoWallet = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:3001/api/wallet-info/upload",
        formData
      );
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`http://localhost:3001/api/wallet-info/${id}`);
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      const response = await axios.put(
        `http://localhost:3001/api/wallet-info/${editingWallet._id}`,
        formData
      );
      setMessage(response.data.message);
      setShowForm(false);
